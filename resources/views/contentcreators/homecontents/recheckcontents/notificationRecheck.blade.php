                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    @if ($n->status == 3)
                                                        ...
                                                        <td>{{$n->dept_id}}</td>
                                                        <td>{{$n->pdf}}</td>
                                                        <td><a class="btn btn-warning" href="editnotification/{{$id=$n->notification_id}}">Edit</a>
                                                        {{-- <n class="btn btn-danger" href="deleteact/{{$id=$n->act_id}}">Delete</n></td> --}}
                                                        <td>
                                                            @if($n->status == 1)
                                                                <form action="{{('/sendtopubnoti/'.$n->notification_id)}}" method="POST" id="formSent">
                                                                    {{ method_field('PUT') }}
                                                                    {{ csrf_field() }}                    
                                                                    <button type="submit" class="btn btn-default" name="changeStatus" value="0" >Sent</button>
                                                                </form>                    
                                                            @else
                                                                <form action="{{('/sendtopubnoti/'.$n->notification_id)}}" method="POST" id="pending">
                                                                    {{ method_field('PUT') }}
                                                                    {{ csrf_field() }}                      
